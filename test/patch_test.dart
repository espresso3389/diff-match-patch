...
main() {
  group('Patch', () {
    test('toString', () {
      ...
      p.length2 = 17;
      p.diffs = [
        Diff(DiffOperation.equal, 'jump'),
        Diff(DiffOperation.delete, 's'),
        Diff(DiffOperation.insert, 'ed'),
    ...
    group('AddContext', () {
      ...
      test('Simple case', () {
        ...
        patchAddContext(p, 'The quick brown fox jumps over the lazy dog.', margin);
        expect(p.toString(), equals('@@ -17,12 +17,18 @@\n fox \n-jump\n+somersault\n s ov\n'));
      });
      test('Not enough trailing context', () {
        var p = patchFromText('@@ -21,4 +21,10 @@\n-jump\n+somersault\n')[0];
        patchAddContext(p, 'The quick brown fox jumps.', margin);
        expect(p.toString(), equals('@@ -17,10 +17,16 @@\n fox \n-jump\n+somersault\n s.\n'));
      });
      ...
      test('Ambiguity', () {
        var p = patchFromText('@@ -3 +3,2 @@\n-e\n+at\n')[0];
        patchAddContext(p, 'The quick brown fox jumps.  The quick brown fox crashes.', margin);
        expect(p.toString(), equals('@@ -1,27 +1,28 @@\n Th\n-e\n+at\n  quick brown fox jumps. \n'));
      });
    });
    ...
    group('Add padding', () {
      test('Both edges full', () {
        ...
        expect(patchToText(patches), equals('@@ -1,8 +1,12 @@\n %01%02%03%04\n+test\n %01%02%03%04\n'));
      });
      test('Both edges partial', () {
        var patches = patchMake('XY', b: 'XtestY');
        expect(patchToText(patches), equals('@@ -1,2 +1,6 @@\n X\n+test\n Y\n'));
        patchAddPadding(patches);
        expect(patchToText(patches), equals('@@ -2,8 +2,12 @@\n %02%03%04X\n+test\n Y%01%02%03\n'));
      });
      test('Both edges none', () {
        var patches = patchMake('XXXXYYYY', b: 'XXXXtestYYYY');
        expect(patchToText(patches), equals('@@ -1,8 +1,12 @@\n XXXX\n+test\n YYYY\n'));
        patchAddPadding(patches);
        expect(patchToText(patches), equals('@@ -5,8 +5,12 @@\n XXXX\n+test\n YYYY\n'));
      });
    });
    ...
    group('Apply', () {
      ...
      test('Exact match', () {
        ...
        expect(resultStr, equals('That quick brown fox jumped over a lazy dog.\ttrue\ttrue'));
      });
      ...
      test('Partial match', () {
        ...
        var boolArray = results[1];
        var resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
        expect(resultStr, equals('That quick red rabbit jumped over a tired tiger.\ttrue\ttrue'));
      });
      ...
      test('Failed match', () {
        ...
        var boolArray = results[1];
        var resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
        expect(resultStr, equals('I am the very model of a modern major general.\tfalse\tfalse'));
      });
      ...
      test('No side effects with major delete', () {
        var patches = patchMake('The quick brown fox jumps over the lazy dog.', b: 'Woof');
        var patchStr = patchToText(patches);
