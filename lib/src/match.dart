...
int matchBitap(String text, String pattern, int loc, double threshold, int distance) {
  ...
  assert(bitsPerInt == 0 || pattern.length <= bitsPerInt);
  ...
  if (bestLoc != -1) {
    scoreThreshold = min(_bitapScore(0, bestLoc, loc, pattern, distance), scoreThreshold);
  ...
  for (var d = 0; d < pattern.length; d++) {
    ...
    binMid = binMax;
    while (binMin < binMid) {
      if (_bitapScore(d, loc + binMid, loc, pattern, distance) <= scoreThreshold) {
        binMin = binMid;
      } else {
        binMax = binMid;
      }
